...
gum_interceptor_transaction_end (GumInterceptorTransaction * self)
{
  ...
  {
    /*
     * stealth hook 无需经过 gum_memory_patch_code_pages (mprotect RW → write)
    ...
    if (has_non_stealth)
    {
      if (!gum_memory_patch_code_pages (addresses, FALSE, gum_apply_updates,
            self))
      {
        g_abort ();
      }
                   ...
                   gpointer user_data)
{
  ...
  for (i = 0; i != pending->len; i++)
  {
    ...
    /* stealth hook 已在 transaction_end 中直接处理, 此处跳过 */
    if (update->ctx->stealth)
      continue;
